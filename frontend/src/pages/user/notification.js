...
export default function Notification() {
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/request/users', {
        method: 'POST',
        headers: {
  ...
  const acceptRequest = async (receivedid) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/accept-request', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const rejectRequest = async (receivedid) => {
    try {
      const response = await fetch('http://localhost:8000/api/reject-request', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
