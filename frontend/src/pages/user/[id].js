...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    const checkFollowerstatus = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8000/api/isfollowing/${userId}/${id}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const follow = async()=>{
    try{
      const res = await fetch('http://localhost:8000/api/follower',{
        method: "PUT",
        headers:{
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
  ...
  const unfollow = async()=>{
    try{
      const res = await fetch('http://localhost:8000/api/unfollower',{
        method: "PUT",
        headers:{
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
