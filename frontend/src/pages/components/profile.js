...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchprofile = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:8000/api/profilepic/${userId}`, { // Use the user ID in the endpoint
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      if (isClient) {
        ...
        try {
          const res = await fetch(`http://localhost:8000/api/userposts/${userId}`, { // Use the user ID in the endpoint
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              authorization: `Bearer ${token}`
  ...
  useEffect(() => {
    const fetchlikes = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8000/api/user/${userId}/liked-posts`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleLike = async (postId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/like/${postId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
  ...
  const handleUnlike = async (postId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/unlike/${postId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
  ...
  const handlecomment = async (text, id) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/${id}/comment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`
